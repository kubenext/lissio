...
// This is a sample plugin showing the features of Lissio's plugin API.
func main() {
	// Remove the prefix from the go logger since Lissio will print logs with timestamps.
	log.SetPrefix("")

	// This plugin is interested in Pods
	podGVK := schema.GroupVersionKind{Version: "v1", Kind: "Pod"}

	// Tell Lissio to call this plugin when printing configuration or tabs for Pods
	...
	// Set up what should happen when Lissio calls this plugin.
	...
	// The plugin can log and the log messages will show up in Lissio.
	log.Printf("lissio-sample-plugin is starting")
	p.Serve()
}

// handleTab is called when Lissio wants to print a tab for an object.
func handleTab(request *service.PrintRequest) (plugin.TabResponse, error) {
	...
	// Lissio uses flex layouts to display information. It's a flexible
	...
	// Lissio contain's a library of components that can be used to display content.
	...
	// In this example, this plugin will tell Lissio to create a new
...
// handlePrint is called when Lissio wants to print an object.
func handlePrint(request *service.PrintRequest) (plugin.PrintResponse, error) {
	...
	// Lissio has a helper function to generate a key from an object. The key
	...
	// Lissio has a component library that can be used to build content for a plugin.
...
// be called frequently from Lissio. Navigation is a tree of `Navigation` structs.
