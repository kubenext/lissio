...
// Request represents a path request from Lissio. It will always be a
...
type Request struct {
	...
	// Path is path that Lissio is requesting. It is scoped to the plugin.
	// i.e. If Lissio wants to render /content/plugin/foo, Path will be
	// `/foo`.
	Path string
}
